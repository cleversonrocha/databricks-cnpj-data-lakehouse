WITH date_spine AS (
    ...
    SELECT explode(sequence(
        (SELECT MIN(data_situacao_cadastral) FROM {{ ref('int_estabelecimentos') }} WHERE data_situacao_cadastral IS NOT NULL),
        (SELECT MAX(data_situacao_cadastral) FROM {{ ref('int_estabelecimentos') }} WHERE data_situacao_cadastral IS NOT NULL),
        INTERVAL 1 DAY
    )) AS data_referencia
),
