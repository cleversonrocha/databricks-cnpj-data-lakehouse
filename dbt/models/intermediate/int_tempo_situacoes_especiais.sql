WITH date_spine AS (
    ...
    SELECT explode(sequence(
        (SELECT MIN(data_situacao_especial) FROM {{ ref('int_estabelecimentos') }} WHERE data_situacao_especial IS NOT NULL),
        (SELECT MAX(data_situacao_especial) FROM {{ ref('int_estabelecimentos') }} WHERE data_situacao_especial IS NOT NULL),
        INTERVAL 1 DAY
    )) AS data_referencia
),
